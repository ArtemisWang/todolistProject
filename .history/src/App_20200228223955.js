...
const App=function(){
    ...
    const [done, setDone]=useState(['undone','undone','undone','undone'])
    ...
    function hasDone(i){
        done[i]=(done[i]==='done'?'undone':'done')
        console.log(done)
        setDone(done)
    }
    ...
    <div className='app'>
        ...
        <div className='u-list'>
            <ul className='todo-list'>
                {list.map((it,i)=>(<li key={it+i} onClick={deleteItem.bind(this,i)} className={done[i]}>{it}</li>))}
            </ul>
        </div>
    </div>)
}
